...
public class JsonResolveUtils {
	...
	// 公告的json文件名称
	private static final String NEARBY_FEED = "nearby_feed.json";
	// 用户资料文件夹
	 ...
	 * @return
	 */
	public static Boolean resolveNearbyFeed(BaseApplication application) {
		if (application.mNearByFeeds != null
				&& application.mNearByFeeds.isEmpty()) {
			String json = TextUtils.getJson(
					application.getApplicationContext(), NEARBY_FEED);
			if (json != null) {
				try {
					JSONArray array = new JSONArray(json);
					...
					for (int i = 0; i < array.length(); i++) {
						...
						application.mNearByFeeds.add(feed);
					}
				} catch (JSONException e) {
					application.mNearByFeeds.clear();
				}
			}
		}
		if (application.mNearByFeeds.isEmpty()) {
			return false;
		} else {
			return true;
		}
